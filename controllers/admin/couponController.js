...
const getCouponPage = async (req, res) => {
  try {
    ...
    const totalDiscounts = coupons.reduce(
      (total, coupon) => total + coupon.used * parseFloat(coupon.value),
      0
    );
...
const addCoupon = async (req, res) => {
  ...
  if (!code || code.length < 5 || code.length > 10) {
    return res
      ...
      .json({ error: 'Coupon code must be between 5 and 10 characters.' });
  }
  ...
  if (!discountAmount || discountAmount <= 0) {
    return res
      .status(HttpStatus.BAD_REQUEST)
      .json({ error: 'Discount amount must be greater than zero.' });
  }
...
const removeCoupon = async (req, res) => {
  try {
    ...
    res
      .status(HttpStatus.INTERNAL_SERVER_ERROR)
      .json({ error: 'Failed to remove coupon' });
  }
};
